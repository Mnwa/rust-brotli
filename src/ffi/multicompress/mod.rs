...
use brotli_decompressor::ffi::interface::{
    CAllocator, brotli_alloc_func, brotli_free_func, c_void,
};
...
use crate::enc::encode::{
    BrotliEncoderOperation, BrotliEncoderParameter, BrotliEncoderStateStruct, set_parameter,
};
...
macro_rules! make_send_alloc {
    ($alloc_func: expr_2021, $free_func: expr_2021, $opaque: expr_2021) => {
        SendAlloc::new(
    ...
    unsafe {
        ...
        compressor::catch_panic(|| {
            ...
            if num_threads == 1 {
                ...
                let m8 = BrotliSubclassableAllocator::new(SubclassableAllocator::new(
                    allocators.clone(),
                ));
            ...
            let res = crate::enc::compress_multi_no_threadpool(
                &params,
                owned_input,
                output_slice,
...
pub struct BrotliEncoderWorkPool {
    ...
    work_pool: crate::enc::WorkerPool<
        crate::enc::CompressionThreadResult<BrotliSubclassableAllocator>,
    ...
    unsafe {
        catch_panic_wstate(|| {
            ...
            let to_box = BrotliEncoderWorkPool {
                ...
                work_pool: crate::enc::new_work_pool(min(num_threads, MAX_THREADS)),
            };
...
pub unsafe extern "C" fn BrotliEncoderDestroyWorkPool(work_pool_ptr: *mut BrotliEncoderWorkPool) {
    unsafe {
        ...
        if let Err(panic_err) = compressor::catch_panic(|| {
            // Capture the whole `UnsafeUnwindBox`, not just its pointer field. Since edition 2021
            // closures capture disjoint fields, which would capture the bare `*mut` and bypass the
            // `RefUnwindSafe` impl the newtype exists to provide.
            let wpp = &wpp;
            if (*wpp.0).custom_allocator.alloc_func.is_some() {
                if let Some(free_fn) = (*wpp.0).custom_allocator.free_func {
                    ...
                    let ptr =
                        core::mem::transmute::<*mut BrotliEncoderWorkPool, *mut c_void>(wpp.0);
...
pub unsafe extern "C" fn BrotliEncoderCompressWorkPool(
    ...
    unsafe {
        ...
        compressor::catch_panic(|| {
            // See `BrotliEncoderDestroyWorkPool`: capture the newtype whole, not its pointer field.
            let work_pool_wrapper = &work_pool_wrapper;
            let null_opaques = [core::ptr::null_mut::<c_void>(); MAX_THREADS];
            let alloc_opaque = if alloc_opaque_per_thread.is_null() {
                &null_opaques[..]
            } else {
            ...
            let res = crate::enc::compress_worker_pool(
                &params,
